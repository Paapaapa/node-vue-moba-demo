      ...
      <swiper>
        <swiper-slide>
          <div>
            <div class="p-3 bg-white border-bottom">
              ...
              <div class="skills my-4">
                ...
                <div v-if="currentSkill">
                  ...
                  <p class="text-gray-1">小提示：{{currentSkill.tips}}</p>
                </div>
              </div>
            </div>

            <m-card noMore icon="caidan" title="出装推荐" class="hero-items">
              ...
              <div class="d-flex my-3 jc-around text-center">
                <div v-for="item in model.items1" :key="item.name">
                  <img :src="item.icon" :alt="item.name" class="icon" />
                  <div>{{item.name}}</div>
                </div>
            ...
            <m-card noMore icon="caidan" title="使用技巧">
              <p class="m-0">{{model.usageTips}}</p>
            </m-card>

            <m-card noMore icon="caidan" title="对抗技巧">
              <p class="m-0">{{model.battleTips}}</p>
            </m-card>

            <m-card noMore icon="caidan" title="团战思路">
              <p class="m-0">{{model.teamTips}}</p>
            </m-card>

            <m-card noMore icon="caidan" title="英雄关系">
              <div class="fs-lg">最佳搭档</div>
              <div v-for="item in model.partners" :key="item.name" class="d-flex pt-3">
                <img :src="item.hero.avatar" :alt="item.hero.name" width="50" height="50" />
                <p class="flex-1 ml-3 m-0 fs-sm">{{item.description}}</p>
              </div>
